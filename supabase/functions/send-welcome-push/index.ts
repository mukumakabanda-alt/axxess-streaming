...
Deno.serve(async (req: Request) => {
  ...
  // Claim this subscription's welcome send first. If the insert fails on
  // the unique constraint, it's already been sent — skip silently.
  const { data: claimed, error: claimErr } = await supabase
    .from("notification_log")
    .insert({
      entity_type: "subscription",
      entity_id: subscriptionId, // not a DB row id here — just used as the unique key for this device
