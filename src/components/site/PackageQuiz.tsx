...
const QUESTIONS: Q[] = [
  {
    ...
    options: [
      { label: "🎬 Movies & series", value: "video" },
      { label: "🎵 Music & podcasts", value: "music" },
      { label: "🎬🎵 A bit of everything", value: "both" },
    ],
  },
  {
    id: "budget",
    ...
    options: [
      { label: "💸 The lowest price", value: "price" },
      { label: "💎 Best overall value", value: "value" },
...
function recommend(a: Record<string, string>): Recommendation {
  if (a.use === "music") {
    return { name: "Spotify", reason: "Pure music — Spotify is your perfect match.", available: true, price: 35, realPrice: 117 };
  }
  if (a.use === "both" || a.budget === "premium") {
    return { name: "All Access Bundle", reason: "Netflix + Spotify in one. Best value, full experience.", available: true, price: 100, realPrice: 315 };
  }
  return { name: "Netflix", reason: "Movies, series, daily streaming — easy on the pocket.", available: true, price: 70, realPrice: 197 };
}
    ...
    <section id="quiz" className="px-4 py-16 sm:px-6">
      <div className="mx-auto max-w-2xl">
        <div className="text-center">
          ...
          <p className="mx-auto mt-2 max-w-xl text-muted-foreground">3 quick questions. We'll match you instantly.</p>
        </div>
        ...
        <div className="mt-8 rounded-3xl border border-border bg-card p-5 shadow-elegant sm:p-8">
          ...
          {!done && (
            <div>
              ...
              {step === 0 && (
                <a href="#plans" className="mt-5 block text-center text-xs text-muted-foreground hover:text-primary">
                  Skip — show me all plans
                </a>
              )}
            </div>
          )}
          ...
          {done && rec && (
            <div className="text-center">
              ...
              <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
                {rec.available ? (
                  <Link to="/trial" className="btn-primary-cta px-7 py-3.5">
